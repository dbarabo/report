package ru.barabo.loan.addfactors.service
...
object AddFactorsService   : StoreFilterService<AddFactors>(AfinaOrm, AddFactors::class.java),
    ParamsSelect, CrossData<AddFactors>, StoreListener<List<ClientBook>> {
    ...
    fun pasteFromTemplate(propColumn: KMutableProperty1<AddFactors, Any?>) {
    ...
    override fun setValue(value: Any?, rowIndex: Int, propColumn: KMutableProperty1<AddFactors, Any?>) {
    ...
    private fun saveTemplate(entity: AddFactors, value: String?) {

        val id = entity.id ?: throw Exception("Не задан id строки AddFactors")

        AfinaQuery.execute(EXEC_SAVE_TEMPLATE, arrayOf(value?:"", id))
    }

    private fun saveRemark(entity: AddFactors, value: String?, onMonth: Timestamp) {
    ...
    private fun saveCheck(entity: AddFactors, value: Int?, onMonth: Timestamp) {
    ...
    private const val EXEC_SAVE_TEMPLATE = "update od.PTKB_LOAN_ADD_FACTORS set TEMPLATE_REMARK = ? where ID = ?"

    private const val EXEC_SAVE_REMARK = "{ call od.PTKB_LOAN_METHOD_JUR.setRemarkAddFactors(?, ?, ?, ?) }"

    private const val EXEC_SAVE_CHECK = "{ call od.PTKB_LOAN_METHOD_JUR.setCheckAddFactors(?, ?, ?, ?) }"

    private const val EXEC_FROM_TEMPLATE = "{ call od.PTKB_LOAN_METHOD_JUR.pasteAddFactorsFromTemplate(?, ?) }"
}