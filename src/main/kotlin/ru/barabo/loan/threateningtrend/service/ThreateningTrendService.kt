package ru.barabo.loan.threateningtrend.service
...
object ThreateningTrendService : StoreFilterService<ThreateningTrend>(AfinaOrm, ThreateningTrend::class.java),
    ParamsSelect, CrossData<ThreateningTrend>, StoreListener<List<ClientBook>> {
    ...
    override fun getRowType(rowIndex: Int): RowType = ThreateningTrendService.dataList[rowIndex].rowType
    ...
    override fun setValue(value: Any?, rowIndex: Int, propColumn: KMutableProperty1<ThreateningTrend, Any?>) {
    ...
    private fun saveTemplate(entity: ThreateningTrend, value: String?) {

        val id = entity.id ?: throw Exception("Не задан id строки ThreateningTrend")

        AfinaQuery.execute(EXEC_SAVE_TEMPLATE, arrayOf(value?:"", id))
    }

    private fun saveRemark(entity: ThreateningTrend, value: String?, onMonth: Timestamp) {
    ...
    private fun saveCheck(entity: ThreateningTrend, value: Int?, onMonth: Timestamp) {
    ...
    private const val EXEC_SAVE_TEMPLATE = "update od.PTKB_LOAN_NEGATIVE_TRENDS set TEMPLATE_REMARK = ? where ID = ?"

    private const val EXEC_SAVE_REMARK = "{ call od.PTKB_LOAN_METHOD_JUR.setRemarkNegativeTrends(?, ?, ?, ?) }"

    private const val EXEC_SAVE_CHECK = "{ call od.PTKB_LOAN_METHOD_JUR.setCheckNegativeTrends(?, ?, ?, ?) }"

    private const val EXEC_FROM_TEMPLATE = "{ call od.PTKB_LOAN_METHOD_JUR.pasteTrendsRemarkFromTemplate(?, ?) }"
}